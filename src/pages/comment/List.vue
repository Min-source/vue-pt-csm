...
export default {
    data(){
        ...
        methods:{
             loadData(){
        let url= 'http://47.100.92.238:5588/comment/findAll'
        ...
        loadOrder(){
            let url="http://47.100.92.238:5588/order/findAll"
            request.get(url).then((response)=>{
        ...
        submitHandler(){
            let url = 'http://47.100.92.238:5588/comment/saveOrUpdate'
            request({
             url,
               method:"POST",
               headers:{
        ...
        deleteHandler(id){
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
              ...
              let url="http://47.100.92.238:5588/comment/deleteById?id="+id;
              request.get(url).then((response)=>{
                this.loadData();
                  this.$message({
                type: 'success',
